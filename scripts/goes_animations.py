...
def get_channel_dataset_names(channel):
    return glob.glob(os.path.join('..', 'data', 'satellite', 'Channel{:02d}/*'.format(channel)))
...
def channel_histogram(channel):
    ...
    ax.hist(first_ds.variables['Sectorized_CMI'][:].compressed().flatten(), bins=255, alpha=0.5)
    ax.hist(last_ds.variables['Sectorized_CMI'][:].compressed().flatten(), bins=255, alpha=0.5)
...
def make_channel_animation(channel):
    ...
    # Pull out projection information from the first file, assume it stays the same through the animation
    ...
    anim.save(os.path.join('..', 'animations', 'GOES16', 'GOES16_Channel_{:02d}.mp4'.format(channel)))
